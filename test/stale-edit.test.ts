...
describe('an edit saved against a file that moved', () => {
  ...
  it('will not let a twin that moved onto the old line take the edit', () => {
    // The edit was opened on the copy under "# H2"; two lines added above have
    // since put the "# H" copy on the very line it came from. The heading each
    // sits under is all that still tells the two apart.
    const opened = parseMarkdown('# H2\n- a\n  a text\n  more', 'Note');
    const now = '# H\n- a\n  a text\n  more\n# H2\n- a\n  a text\n  more';
    const at = relocateBodyEdit(
    ...
    expect(at?.anchor).toBe(6);
    expect(at?.node.parent?.text).toBe('H2');
  });
